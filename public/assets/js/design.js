        ...
        .then((images) => {
            if (images.length > 0) {
                ...
                if (images[0].crop === 1) {
                    ...
                    const inheritDesignDiv = document.querySelector(
                        "#frameWrap .inherit-design"
                    );
                    if (inheritDesignDiv) {
                ...
                document.querySelector(".file-uploadSection").style.display =
                    "flex";
                document.querySelector(".FrameDesignSection").style.display =
                    "none";
            }
        })
...
function renderSliderImages(imagesArray) {
    const swiperWrapper = document.querySelector(
        ".Images-frame-slider .swiper-wrapper"
    );
...
function applyInitialFrameDesign(imageObj) {
    ...
    const initialDesignClass =
        frameConfig.design?.designClass || "classic-card-design";
    ...
    const designOptions = document.querySelectorAll(
        ".frame-change.dropdown-item"
    );
...
function applyInitialFrameSize(imageObj) {
    ...
    if (framePriceShow) {
        framePriceShow.textContent =
            initialSize.frame_price > 0
                ? `$${initialSize.frame_price}`
                : "Free";
    }
}
...
function applyInitialFrameFinish(imageObj) {
    ...
    const finishOptions = document.querySelectorAll(
        ".frame-finish.dropdown-item"
    );
    ...
    .addEventListener("click", async function () {
        ...
        if (activeSlide) {
            ...
            if (imgElement) {
                ...
                const imageName = imageNameWithExt
                    .split(".")
                    .slice(0, -1)
                    .join(".");
                ...
                if (confirmDelete.isConfirmed) {
                    const totalSlides =
                        document.querySelectorAll(".swiper-slide img").length;
                    ...
                    fetch(get_frame_config, {
                        ...
                        .then((data) => {
                            if (data.success) {
                                ...
                                applyInitialFrameDesign(
                                    data.frame_configuration
                                );
                                applyInitialFrameColor(
                                    data.frame_configuration
                                );
                                applyInitialFrameSize(data.frame_configuration);
                                applyInitialFrameFinish(
                                    data.frame_configuration
                                );
                                ...
                                console.error(
                                    "Failed to fetch frame configuration:",
                                    data.message
                                );
                            }
                        })
                        .catch((err) =>
                            console.error(
                                "Error fetching frame configuration:",
                                err
                            )
                        );
                }
...
document
    .querySelector(".Images-frame-slider .swiper-wrapper")
    .addEventListener("click", function (e) {
        const slide = e.target.closest(".swiper-slide");
        if (!slide) return;
        document
            .querySelectorAll(".Images-frame-slider .swiper-slide")
            .forEach(function (s) {
...
sizeOptions.forEach((option) => {
    option.addEventListener("click", function () {
        ...
        const frameSizeText =
            this.querySelector(".propertyName").textContent.trim();
...
function saveFrameConfigToDatabase(frameConfig, type) {
    ...
    if (activeSlide) {
        ...
        if (activeImg) {
            ...
            const imageName = activeSrc
                .split("/")
                .pop()
                .split(".")
                .slice(0, -1)
                .join("."); // Just the file name (image.jpg)
...
async function sendFrameConfigToServer(imageName, frameConfig, type) {
    ...
    if (result.success) {
        ...
        document.getElementById("active_config").value = JSON.stringify(
            result.data
        );
    } else {
...
function saveFrameConfigInDB(imageUrl) {
    const config =
        JSON.parse(localStorage.getItem("frameConfigurations")) || {};
            ...
            fetch(config_url, {
                ...
                .then((data) => {
                    if (data.success) {
                        ...
                        console.error(
                            "Error saving configuration to DB",
                            data.message
                        );
                    }
                })
    ...
    $("#cropImageBtn").on("click", function () {
        ...
        $uploadCrop
            ...
            .then(function (resp) {
                ...
                const filename = imgElement
                    ? imgElement.getAttribute("data-frame-config")
                    : "default.jpg";
